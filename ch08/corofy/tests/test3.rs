...
use corofy::rewrite;
#[test]
fn produces_expected_output_3() {
    let src = fs::read_to_string("./tests/test3/input.txt").unwrap();
    let dest_path = temp_dir().join("test3.txt");
    ...
    let expected = fs::read_to_string("./tests/test3/expected.txt").unwrap();
