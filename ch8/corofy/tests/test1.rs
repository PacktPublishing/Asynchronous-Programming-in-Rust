...
use corofy::rewrite;
#[test]
fn produces_expected_output_1() {
    let src = fs::read_to_string("./tests/test1/input.txt").unwrap();
    let dest_path = temp_dir().join("test1.txt");
    ...
    let expected = fs::read_to_string("./tests/test1/expected.txt").unwrap();
